...
const Form = React.createClass({
  ...
  render() {
    ...
    const formComponents = this.state.data.fields.map( (field, idx) => {
      if (field.type === 'text') {
        return <InputText field={field} key={idx}/>
      } else if (field.type === 'radio') {
        return <InputRadios field={field} key={idx}/>
      } else if (field.type === 'checkbox') {
        return <InputCheckboxes field={field} key={idx}/>
      } else if (field.type === 'textarea') {
        return <Textarea field={field} key={idx}/>
      }
    })
    return (
      <div>
